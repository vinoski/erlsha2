...
hex_int(binary,Binary)->
    binary_to_integer(hmac:hexlify_to_bin(Binary), 16).
...
wikipedia_test() ->
    [ begin
          ...
          ?assertMatch(16#b936cee86c9f87aa5d3c6f2e84cb5a4239a5fe50480a6ec66b70ab5b1f4ac6730c6c515421b327ec1d69402e53dfb49ad7381eb067b338fd7b0cb22247225d47,
                       hex_int(Type, hmac:hmac512("", ""))),
          ?assertMatch(16#de7c9b85b8b78aa6bc8a7a36f70a90701c9db4d9,
                       hex_int(Type, hmac:hmac("key", "The quick brown fox jumps over the lazy dog"))),
          ?assertMatch(16#f7bc83f430538424b13298e6aa6fb143ef4d59a14946175997479dbc2d1a3cd8,
                       hex_int(Type, hmac:hmac256("key", "The quick brown fox jumps over the lazy dog"))),
          ?assertMatch(16#b42af09057bac1e2d41708e48a902e09b5ff7f12ab428a4fe86653c73dd248fb82f948a549f7b791a5b41915ee4d1ec3935357e4e2317250d0372afa2ebeeb3a,
                       hex_int(Type, hmac:hmac512("key", "The quick brown fox jumps over the lazy dog")))
      end || Type <- [ list, binary] ],
    ok.
...
rfc_4231_1_test() ->
    ...
    [ begin
          ...
          ?assertMatch(16#87aa7cdea5ef619d4ff0b4241a1d6cb02379f4e2ce4ec2787ad0b30545e17cdedaa833b7d6b8a702038b274eaea3f4e4be9d914eeb61f1702e696c203a126854,
                       hex_int(Type, hmac:hmac512(Key, Val)))
      end || Type <- [ list, binary] ],
    ok.
...
rfc_4231_5_test() ->
    ...
    [ begin
          ?assertMatch(16#0e2aea68a90c8d37c988bcdb9fca6fa8, hex_int(Type, Left224)),
          ?assertMatch(16#a3b6167473100ee06e0c796c2955552b, hex_int(Type, Left256)),
          ?assertMatch(16#3abf34c3503b2a23a46efc619baef897, hex_int(Type, Left384)),
          ?assertMatch(16#415fad6271580a531d4179bc891d87a6, hex_int(Type, Left512))
      end || Type <- [ list, binary] ],
    ok.
